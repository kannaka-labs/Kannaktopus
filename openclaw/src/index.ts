...
/**
 * Manifest config keys that nothing consumes (issue #31).
 *
 * Every workflow runs through the one local `execFile` path, so a user who sets
 * `mcpServerUrl` gets no error and no effect — their workflows execute locally
 * while they believe they are running remotely. That is a silent lie about
 * where the code ran, and it is worse than an absent key. `claudeCodePath` has
 * the identical defect. Remote MCP transport is a feature with its own design
 * (connection lifecycle, unreachable-server behaviour, streaming a 300s
 * orchestration) and is deliberately not implemented here.
 *
 * Registration refuses rather than each call refusing, so the user finds out at
 * load time instead of midway through their first workflow.
 */
interface UnsupportedConfigKey {
  key: string;
  /**
   * Values that mean "not configured". openclaw.plugin.json declares a schema
   * default for each of these keys, so a host that materializes its defaults
   * into pluginConfig would otherwise trip the refusal on a stock install.
   */
  unsetValues: string[];
  reason: string;
}

const UNSUPPORTED_CONFIG_KEYS: UnsupportedConfigKey[] = [
  {
    key: "mcpServerUrl",
    unsetValues: [""],
    reason:
      "remote MCP is not supported yet; unset mcpServerUrl to use local execution. " +
  ...
  },
  {
    ...
    unsetValues: ["", "claude"],
    reason:
      "claudeCodePath is read by nothing; execution always goes through orchestrate.sh. " +
 ...
 */
function assertNoUnsupportedConfig(
  pluginConfig: Record<string, unknown>,
  logger: PluginLogger
): void {
  for (const { key, unsetValues, reason } of UNSUPPORTED_CONFIG_KEYS) {
    ...
    if (unsetValues.includes(trimmed)) continue;
